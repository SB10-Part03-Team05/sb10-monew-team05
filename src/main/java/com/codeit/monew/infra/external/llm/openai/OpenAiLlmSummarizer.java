package com.codeit.monew.infra.external.llm.openai;
...
public class OpenAiLlmSummarizer implements LlmSummarizer {
  ...
  public String summarize(String bodyText) {
    ...
    try {
      ...
      try {
        ...
        log.debug(
            "[LLM-OpenAI] Summary completed. Tokens: [In: {}, Out: {}, Total: {}]\nSummary: {}",
            usage.getPromptTokens(),
            usage.getCompletionTokens(),
            usage.getTotalTokens(),
            summary);
      } catch (Exception e) {
        log.debug("[LLM-OpenAI] Summary completed. Tokens: [unavailable]\nSummary: {}", summary);
      }

      return summary;
    } catch (RuntimeException e) {
      throw new ExternalLlmProviderException("openai", null, e);
    }
  }
